...
open class DNSServer: DNSResolverDelegate, IPStackProtocol {
    ...
    fileprivate var fakeSessions: [Address: DNSSession] = [:]
     ...
     */
    fileprivate func cleanUpFakeIP(_ address: Address, after delay: Int) {
    ...
    func isFakeIP(_ ipAddress: Address) -> Bool {
        return pool?.contains(ip: ipAddress) ?? false
    }

    func lookupFakeIP(_ address: Address) -> DNSSession? {
